import { useAddSkillsMutation } from "@/services/profile";
...
const SkillsCard = ({ skills }: SkillsCardProps) => {
	...
	const addEduca = () => {
		setShowSkillsCard(!showSkillsCard);
	};
	...
	const [addSkills, { isLoading: loading }] = useAddSkillsMutation();

	const addSkillsFunction = async (data: any) => {
		try {
			const details = {
			...
			const response = await addSkills(details).unwrap();
	...
	return (
		<WhiteWrapper>
			...
			<Modal show={showSkillsCard} onRequestClose={() => addEduca()}>
				<FormProvider {...methods}>
					<form onSubmit={handleSubmit(addSkillsFunction)}>
						...
						<div className='mt-6 space-y-10'>
							<div>
								<Input label='Skill ' type='text' name='skills' required />
							</div>
						</div>
						<div className='mt-6'>
