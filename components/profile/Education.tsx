...
const Education = ({ educations }: any) => {
	const month = [
		"January",
		"February",
		"March",
	...
	return (
		<div>
			...
			{educations &&
				educations.slice(0, visible).map((education: any, index: any) => {
					console.log(education);
					const formatEndDate = new Date(`${education?.endDate}`);
					const formatStartDate = new Date(`${education?.startDate}`);
					const { _id: id } = education;
					console.log(id);
						...
						<React.Fragment key={index}>
							<article
								className={`flex gap-6 py-6 ${
									index + 1 === visible || index + 1 === educations.length
										? ""
								...
								<div className='relative'>
									<div className='space-y-2 relative'>
										<div className='font-semibold flex justify-between text-title'>
											{education.school}{" "}
											<button onClick={() => deleteEducation({ id, token })}>
												del
											</button>
										</div>
							...
							{showLess && index === educations.length - 1 && (
								<div className='flex justify-center  mt-5'>
									<Button
										onClick={loadLess}
										theme='plain'
			...
			<Modal show={showAddEducation} onRequestClose={() => addEduca()}>
